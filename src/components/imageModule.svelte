...
<script lang="ts">
  ...
  export const isInFrame = () => {
    if (itemRef.parentElement !== null) {
      if (
        itemRef.parentElement.getBoundingClientRect().top < viewportHeight &&
        itemRef.parentElement.getBoundingClientRect().top > viewportHeight - 200
      ) {
        itemRef.classList.add('anim')
      }
      if (
        itemRef.parentElement.getBoundingClientRect().bottom < 0 ||
        itemRef.parentElement.getBoundingClientRect().top > viewportHeight
      ) {
...
<style>
  ...
  .box {
    ...
    height: 70vh;
    transition: transform cubic-bezier(0.37, 0, 0.63, 1) 1000ms;
  }
  .zoom {
    max-width: 100%;
    max-height: 100%;
    display: block;
    transition: transform cubic-bezier(0.65, 0, 0.35, 1) 1000ms;
